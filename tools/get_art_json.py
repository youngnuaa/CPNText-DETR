...
def main(args):
    ...
    with torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            ...
            #print(len(inputs))
            img_infos = inputs[0]
            #print(img_infos["file_name"][58:-4])
            res_name = img_infos["file_name"][58:-4]

            results = model(inputs)[0]
            instances = results["instances"]
            pre_scores = instances.scores
            pre_polygons = instances.polygons
            pre_scores = pre_scores.cpu().numpy()
            pre_polygons = pre_polygons.cpu().numpy()
            pre_polygons = pre_polygons.astype(np.int64)
            #print(pre_scores)
            #print(pre_polygons)
            data_list = []

            for index in range(len(pre_scores)):
                pre_data = {}
                pre_score = pre_scores[index]
                pre_polygon = pre_polygons[index].reshape((-1, 2))  #16, 2

                pre_polygon = pre_polygon.tolist()
                pre_data["points"] = pre_polygon
                pre_data["confidence"] = float(pre_score)
                data_list.append(pre_data)

            data_dict["res_"+res_name] = data_list

    with open("./detection_result.json", mode='w+') as f_json_out:
        json.dump(data_dict, f_json_out, cls=NumpyEncoder)
