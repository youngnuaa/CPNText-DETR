...
def write_json(dict, jsonpath, encoding='utf-8'):
    import json
    jsonpath = jsonpath.strip()
    assert jsonpath.split('.')[-1] == 'json', f'{jsonpath} is not json file !\n'
    json_file = open(jsonpath, 'w', encoding=encoding)
    json.dump(dict, json_file, indent=4, ensure_ascii=False, cls=NumpyEncoder)
    json_file.close()
...
def gen_json(img, polygons, file_name, json_path):

    labelme_dict = {'version':'3.16.1', 'flags':{},'shapes':[], 'lineColor':[0,255,0,128], 'fillColor':[255,0,0,128],
                    'imagePath':'imagepath', 'imageData':'imagedata', 'imageHeight': 0, 'imageWidth': 0}
    ...
    for polygon in polygons:
        ...
        label_dict["points"] = polygon.tolist()
        labelme_dict["shapes"].append(label_dict)

    labelme_dict['imageData'] = img_arr_to_b64(img).decode('utf-8')
    labelme_dict['imageHeight'] = img_h
    labelme_dict['imageWidth'] = img_w
    labelme_dict['imagePath'] = file_name
    write_json(labelme_dict, json_path, "GBK")
...
def main(args):
    ...
    img_name = "gt_1636.jpg"

    with torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            img_infro = inputs[0]
            file_name = img_infro["file_name"]
            if file_name[-11:] == "gt_1636.jpg":

                json_path = "../img/gt_1636.json"
                ...
                polygons = polygons.reshape(polygons.shape[0], 16, 2)
                polygons = polygons.cpu().numpy()
                print(polygons)
                gen_json(img, polygons, "gt_1636.jpg", json_path)
