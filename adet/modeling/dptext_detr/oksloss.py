...
def oks_overlaps(kpt_preds, kpt_gts,  kpt_areas, sigmas):
    ...
    squared_distance0 = squared_distance / ((kpt_areas+1e-6) * variances[None, :] * 2)  #n, k_p
    squared_distance1 = torch.exp(-squared_distance0)
    squared_distance1 = squared_distance1
    oks = squared_distance1.sum(dim=1) / kpt_areas.shape[1]
...
def oks_loss(pred,
    ...
    """
    oks = oks_overlaps(pred, target, area, sigmas).clamp(min=eps)
    if linear:
        loss = 1 - oks
    else:
        loss = -oks.log()
