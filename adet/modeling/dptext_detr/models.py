...
class DPText_DETR(nn.Module):
    def __init__(self, cfg, backbone):
        ...
        #print("###############################################")
        #print("num_proposals", self.num_proposals)
        ...
        self.bbox_coord = MLP(self.d_model, self.d_model, 6, 3)
        self.bbox_class = nn.Linear(self.d_model, self.num_classes)
        self.strides = cfg.MODEL.TRANSFORMER.STRIDES
        self.num_channels = cfg.MODEL.TRANSFORMER.NUM_CHANNELS

        if self.num_feature_levels > 1:
            num_backbone_outs = len(self.strides)
            input_proj_list = []
    ...
    def forward(self, samples: NestedTensor):
        ...
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact = self.transformer(
            srcs, masks, pos, self.ctrl_point_embed.weight)
