...
def detector_box_postprocess(bbx, image_size, output_height, output_width):
...
class TransformerPureDetector(nn.Module):
    def __init__(self, cfg):
        ...
        weight_dict = {'loss_ce': loss_cfg.POINT_CLASS_WEIGHT,
                       ...
                       "loss_point_giou": loss_cfg.POINT_GIOU_WEIGHT,
                       "loss_point_oks": loss_cfg.POINT_OKS_WEIGHT}
        enc_weight_dict = {
        ...
        self.criterion = SetCriterion(
            ...
            point_box_index=loss_cfg.POINT_BOX_INDEX,
            box_index=loss_cfg.BOX_INDEX,
        )
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            enc_outputs_class = enc_outputs["pred_logits"]
            enc_outputs_coord = enc_outputs["pred_boxes"]
            box_results = self.inference_box(enc_outputs_class, enc_outputs_coord, images.image_sizes)
            for results_box_per_image, input_per_image, image_size in zip(box_results, batched_inputs, images.image_sizes):
                bbox = results_box_per_image["bbox"]
                bb_score = results_box_per_image["bb_score"]
                height = input_per_image.get("height", image_size[0])
                width = input_per_image.get("width", image_size[1])
                bbox = detector_box_postprocess(bbox, image_size, height, width)
                processed_results.append({"bbox": bbox, "bb_score":bb_score})
            """
    ...
    def prepare_targets(self, targets):
        ...
        for targets_per_image in targets:
            ...
            gt_areas = gt_areas/(w*h)
            gt_boxes = box_xyxy_to_cxcywh(gt_boxes)
            raw_ctrl_points = targets_per_image.polygons
            ...
            weights = torch.ones_like(gt_ctrl_points[..., 0])

            mean_x = gt_ctrl_points[:, :, 0].mean(dim=-1)
            mean_y = gt_ctrl_points[:, :, 1].mean(dim=-1)
            l_w = mean_x - (gt_boxes[:, 0] - gt_boxes[:, 2]/2)
            l_h = mean_y - (gt_boxes[:, 1] - gt_boxes[:, 3]/2)
            r_w = (gt_boxes[:, 0] + gt_boxes[:, 2]/2) - mean_x
            r_h = (gt_boxes[:, 1] + gt_boxes[:, 3]/2) - mean_y

            gt_boxes = torch.cat((mean_x.unsqueeze(-1), mean_y.unsqueeze(-1),
                                      l_w.unsqueeze(-1), l_h.unsqueeze(-1),
                                      r_w.unsqueeze(-1), r_h.unsqueeze(-1), ), dim=-1)
    ...
    def init_prior_points_from_anchor(self, reference_points_anchor):
        # reference_points_anchor: bs, nq, 6 ==> bs, nq, (cx, cy, lw, lh, rw, rh)
        # return size:
    ...
    def get_oks_sigmas(self, box, control_points):
        """
        box shape n, 6
        control_points n, 16
        """
        reference_points = self.init_prior_points_from_anchor
    ...
    def inference_box(self, enc_outputs_class, enc_outputs_coord, image_sizes):
        """
        ...
        results = []
        for scores_per_image, box_per_image, image_size in zip(enc_outputs_class, enc_outputs_coord, image_sizes):
            selector = scores_per_image >= self.test_score_threshold
            box_info = {}
            scores_per_image = scores_per_image[selector]
            box_per_image = box_per_image[selector]
            x1 = (box_per_image[..., 0] - box_per_image[..., 2])*image_size[1]
            y1 = (box_per_image[..., 1] - box_per_image[..., 3])*image_size[0]
            x2 = (box_per_image[..., 0] + box_per_image[..., 4])*image_size[1]
            y2 = (box_per_image[..., 1] + box_per_image[..., 5])*image_size[0]
            box = torch.stack((x1, y1, x2, y2), dim=-1)
            box_info["bb_score"] = scores_per_image
            box_info["bbox"] = box
