...
class DeformableTransformer_Det(nn.Module):
    ...
    def gen_encoder_output_proposals(self, memory, memory_padding_mask, spatial_shapes):
        ...
        for lvl, (H_, W_) in enumerate(spatial_shapes):
            ...
            proposal = torch.cat((grid, wh, wh), -1).view(N_, -1, 6)
    ...
    def init_prior_points_from_anchor(self, reference_points_anchor):
        ...
        reference_points_anchor_lw = reference_points_anchor[..., 0] - reference_points_anchor[..., 2]*1.03
        reference_points_anchor_rw = reference_points_anchor[..., 0] + reference_points_anchor[..., 4]*1.03
        reference_points_anchor_lw = torch.clamp(reference_points_anchor_lw, 0, 1)
        reference_points_anchor_rw = torch.clamp(reference_points_anchor_rw, 0, 1)
        reference_points_anchor[..., 2] = reference_points_anchor[..., 0] - reference_points_anchor_lw
        reference_points_anchor[..., 4] = reference_points_anchor_rw - reference_points_anchor[..., 0]
        """

        """
        reference_points_anchor[..., 3] = reference_points_anchor[..., 3]*0.975
        reference_points_anchor[..., 5] = reference_points_anchor[..., 5]*0.975
        """
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed):
        ...
        reference_points = topk_coords_unact.sigmoid()  # (bs, nq, 6)

        topk_pos_embed = torch.gather(output_memory, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, self.d_model))

        query_embed = query_embed[None, None, :, :] + topk_pos_embed[:, :, None, :] # bs, n_q, n_p, dim


        #query_embed = query_embed.reshape(self.num_proposals, self.num_ctrl_points, -1)
        #query_embed = query_embed.unsqueeze(0).repeat(bs, 1, 1, 1)
        #reference_points = self.init_control_points_from_anchor(reference_points)  # Prior Points Sampling
        reference_points = self.init_prior_points_from_anchor(reference_points)
...
class DeformableTransformerDecoderLayer_Det(nn.Module):
    ...
    def forward(
        ...
        # intra-group self-attention
        bs, n_q, n_pts, dim = tgt.shape
        shortcut = tgt
        q = k = self.with_pos_embed(tgt, query_pos)

        tgt2_inter = self.attn_inter(
            q.mean(2).transpose(0, 1),
            k.mean(2).transpose(0, 1),
            tgt.mean(2).transpose(0, 1)
        )[0].transpose(0, 1).reshape(bs, n_q, dim)

        tgt_inter = tgt.mean(2) + self.dropout_inter(tgt2_inter)
        tgt_inter = self.norm_inter(tgt_inter).softmax(dim=1)  #(bs, n_q, dim)
        ...
        tgt_circonv = self.drop_path(self.circonv(shortcut+query_pos)*tgt_inter[:, :, None, :])
        tgt = shortcut + self.norm_intra(self.drop_path(tgt*tgt_inter[:, :, None, :]) + tgt_circonv)

        #tgt_circonv = self.drop_path(self.circonv(shortcut+query_pos))
        #tgt = shortcut + self.norm_intra(self.drop_path(tgt) + tgt_circonv)*tgt_inter[:, :, None, :]
        tgt_inter = tgt + self.drop_path(self.norm_fuse(self.mlp_fuse(tgt)))
        tgt_inter = self.norm_self(tgt_inter)
    ...
    def forward(
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.return_intermediate:
                ...
                # look twice
                intermediate_reference_points.append(tmp)
